...
const emptyForm = { name: '', email: '', password: '', rollNumber: '', department: '' };
...
export default function ManageStudents() {
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ ...emptyForm, password: 'student123' });
    setModalOpen(true);
  };

  const openEdit = (u) => {
    setEditing(u);
    setForm({ name: u.name, email: u.email, password: '', rollNumber: u.rollNumber || '', department: u.department || '' });
    setModalOpen(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editing) {
        const { data } = await userAPI.update(editing._id, { name: form.name, email: form.email, rollNumber: form.rollNumber, department: form.department });
        setUsers(prev => prev.map(u => u._id === editing._id ? data.user : u));
        toast.success('Student updated.');
      } else {
        const { data } = await userAPI.create({ ...form, role: 'student' });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="font-display font-bold text-2xl text-gray-900 dark:text-white">Manage Students</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{users.length} students</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
              <tr>
                {['Name', 'Email', 'Roll No.', 'Department', 'Actions'].map(h => (
                ...
                <motion.tr key={u._id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.02 }}
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <div className="w-7 h-7 rounded-full bg-green-100 dark:bg-green-900/30 flex items-center justify-center text-xs font-bold text-green-700">
                  ...
                  <td className="px-4 py-3 text-gray-500 dark:text-gray-400 font-mono text-xs">{u.rollNumber || '—'}</td>
